...
def svm_classifier_train(df, s, ranked_feature):
    ...
    for number, rank in enumerate(all_ranked_feature):
        ...
        search = GridSearchCV(estimator = SVC(), param_grid = params, cv = cv, scoring = 'roc_auc', n_jobs = -1)
        ...
        if search.best_estimator_.kernel == 'rbf':
            ...
            training[number + 1,3] = 'None'
            
            training[number + 1,4] = 'None'
...
def svm_classifier_validation(df, s, ranked_feature, training):
    ...
    for number, rank in enumerate(all_ranked_feature):
        ...
        for n in patient:
            ...
            try:
                ...
                if number == 0:
                   ...
                   globals()['training_%s' % n] = svm_classifier_train(X_train, s, ranked_feature)
                ...
                clf = SVC(kernel = 'rbf', max_iter = -1, C = globals()['training_%s' % n][number + 1, 2], gamma = globals()['training_%s' % n][number + 1, 3], 
                          
                      class_weight = 'balanced', probability = True, verbose = 1).fit(X_resampled_rank, y_resampled.ravel())
                ...
                fpr, tpr, threshold_1 = metrics.roc_curve(y_test, y_pred)
                                                                     
                y_pred = np.where(y_pred > threshold_1[np.argmax(tpr - fpr)], 1, 0)
                                    
                a = metrics.roc_auc_score(y_test, y_pred)
        
                y_pred = clf.predict_proba(X_test)[:,1]
                
                precision, recall, threshold_2 = metrics.precision_recall_curve(y_test, y_pred)
        ...
        result = pd.concat((pd.DataFrame({'auc': auc}), pd.DataFrame({'fscore': fscore}), 
                              
                   pd.DataFrame([''.join(str(rank))[2:-2] for i in range(len(auc))], columns = {'rank'})), axis = 1)
        ...
        del auc, fscore, clf, X_test, X_resampled_rank, n
...
def svm_classifier_test(df, s, ranked_feature, training):
    ...
    for number, rank in enumerate(all_ranked_feature):
        ...
        print('Testing SVM in the %s with feature - %s' %(''.join(s),', '.join(rank)))
        
        X_resampled_rank = X_resampled[rank].to_numpy()
            
        clf = SVC(kernel = 'rbf', max_iter = -1, C = training[number, 2], gamma = training[number, 3],
                  
                  class_weight = 'balanced', verbose = 1).fit(X_resampled_rank, y_resampled.ravel())
